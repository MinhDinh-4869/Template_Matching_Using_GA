...
m = 20 #size of population #must be even num because ò elitism=2
generation = 100
...
def sort_population(population):
	...
	for i in range(m):
		for j in range(i+1,m):
			if (computeFitness(sorted_population[i]) > computeFitness(sorted_population[j])):
...
def generate_new_population(old_population, elitism, gen):
	...
	if gen%1 ==0:
		a = computeFitness(sorted_population[m-1])
		fitness.append(a)
